    ...
    for (let i = 0, firstCharCode, words, l = phrases.length; i < l; i++) {
      ...
      if(PINYIN_DICT[firstCharCode]){
        ...
        if (words.length === 1) {
          ...
          _hans = _hans.concat([words]);
        } else {
          let res = this.phrases_pinyin(words, options);
          pys = pys.concat(res.py);
          _hans = _hans.concat(res.hans);
        }
      } else {
        nohans += words;
      }
...
function segment(hans) {
  ...
  // 词语拼音库。
  PHRASES_DICT = PHRASES_DICT || require("../data/phrases-dict-all");
